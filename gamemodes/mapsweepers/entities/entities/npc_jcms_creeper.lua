...
if SERVER then
	...
	function ENT:Think()
		...
		if selfTbl:GetIsExploding() and cTime > selfTbl:GetExplodeTime() then
			...
			if IsValid(enemy) then
				...
				if viscon and self:GetPos():DistToSqr( self:GetEnemyLastKnownPos(enemy) ) < 2000^2 then
	...
	function ENT:Burst()
		...
		jcms.npc_Spawn("zombie_creep", self:GetPos())
	end
end
...
if CLIENT then
	...
	function ENT:GetCSModel()
		if not IsValid(self.csmodel) then
			self.csmodel = ClientsideModel("models/Zombie/Fast.mdl")
